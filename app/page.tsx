    ...
    <main className="h-screen bg-[#0b0f1a] text-white flex overflow-hidden font-sans">
      ...
      <aside className="w-80 bg-[#111827] border-r border-white/5 flex flex-col shadow-2xl">
        <div className="p-8 border-b border-white/5 bg-black/20">
          ...
          <p className="text-xs text-slate-500 font-bold uppercase mt-2 tracking-widest">Wafaa Agent ✅</p>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {leads.map((l) => (
            <div key={l.id} onClick={() => {setActiveLead(l); setCurrentNote(l.note)}} className={`p-6 rounded-[1.5rem] border-2 transition-all cursor-pointer ${activeLead?.id === l.id ? 'bg-blue-600 border-blue-400' : 'bg-white/5 border-transparent hover:border-white/10'}`}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="h-24 border-b border-white/10 flex items-center justify-between px-10 bg-[#0b0f1a]">
          ...
          <div className="flex gap-4">
            <button onClick={() => setShowDialer(true)} className="bg-emerald-600 px-6 py-3 rounded-2xl font-black uppercase text-xs shadow-lg shadow-emerald-900/40 hover:bg-emerald-500">🎧 Appel Manuel</button>
            <button onClick={() => setShowChat(!showChat)} className="bg-slate-800 px-6 py-3 rounded-2xl font-black uppercase text-xs text-slate-300">💬 Chat Équipe</button>
          </div>
        </header>
          ...
          {view === "AGENT" ? (
            <div className="max-w-7xl mx-auto grid grid-cols-12 gap-10">
              ...
              <div className="col-span-8 space-y-8">
                <div className="bg-[#111827] p-12 rounded-[4rem] border border-white/5 shadow-3xl">
                  <div className="flex justify-between items-start mb-12">
                    ...
                    <div className="bg-emerald-500/10 border-2 border-emerald-500/30 px-10 py-6 rounded-[2.5rem]">
                      <span className="text-5xl font-mono font-black text-emerald-400 tracking-tighter">{activeLead.phone}</span>
                    </div>
                  </div>
                ...
                <div className="bg-[#111827] p-10 rounded-[3rem] border border-white/5">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={saveNote} className="bg-blue-600 hover:bg-blue-500 px-8 py-3 rounded-xl font-black uppercase text-xs shadow-lg transition-all">💾 Enregistrer la fiche</button>
                  </div>
                  <textarea 
                    value={currentNote}
                    onChange={(e) => setCurrentNote(e.target.value)}
                    className="w-full bg-black/40 border-2 border-white/5 rounded-[2rem] p-8 text-lg italic min-h-[200px] outline-none focus:border-blue-500" 
              ...
              <div className="col-span-4 space-y-4">
                <button onClick={() => alert("VENTE ENREGISTRÉE")} className="w-full py-16 bg-emerald-600 hover:bg-emerald-500 rounded-[3rem] font-black text-white uppercase italic text-5xl shadow-2xl transition-all active:scale-95">VENTE ✅</button>
                <button onClick={() => alert("RAPPEL PROGRAMMÉ")} className="w-full py-16 bg-blue-600 hover:bg-blue-500 rounded-[3rem] font-black text-white uppercase italic text-5xl shadow-2xl transition-all active:scale-95">RAPPEL 📅</button>
                <div className="grid grid-cols-1 gap-4 mt-10">
                  <button className="py-6 bg-slate-800 rounded-3xl font-black uppercase text-sm tracking-widest hover:bg-rose-600 transition-colors">Absent / NRP</button>
                  <button className="py-6 bg-slate-800 rounded-3xl font-black uppercase text-sm tracking-widest hover:bg-orange-600 transition-colors">Barrage Sec</button>
                </div>
              </div>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto bg-purple-600/10 border-2 border-purple-500/20 p-20 rounded-[4rem] text-center">
              <h2 className="text-6xl font-black uppercase italic text-purple-400">Dashboard Superviseur</h2>
              <p className="mt-4 text-slate-400 font-bold">Statistiques en temps réel bientôt disponibles.</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showDialer && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl flex items-center justify-center z-[300]">
          <div className="bg-[#111827] p-12 rounded-[4rem] border border-white/10 w-full max-w-md shadow-3xl">
            <div className="flex justify-between mb-10"><span className="text-emerald-400 font-black uppercase text-xs tracking-widest">Dialer Manuel</span><button onClick={() => setShowDialer(false)} className="text-4xl text-slate-600 hover:text-white">✕</button></div>
            ...
            <div className="grid grid-cols-3 gap-6 mb-10">
              {["1","2","3","4","5","6","7","8","9","*","0","#"].map(n => (
                <button key={n} onClick={() => setManualNumber(p => p+n)} className="h-24 w-24 bg-white/5 hover:bg-white/10 rounded-full text-3xl font-black transition-all active:bg-emerald-600">{n}</button>
              ))}
            </div>
            <button onClick={() => {alert("Appel en cours vers " + manualNumber); setShowDialer(false)}} className="w-full py-8 bg-emerald-600 rounded-[2rem] font-black text-2xl uppercase italic shadow-xl hover:bg-emerald-500">Appeler 📞</button>
            <button onClick={() => setManualNumber("")} className="w-full mt-4 text-xs font-black text-slate-500 uppercase hover:text-white">Effacer tout</button>
          </div>
        </div>
      )}

      {/* PANEL CHAT */}
      <aside className={`fixed top-0 right-0 h-full w-[450px] bg-[#111827] border-l-2 border-white/10 z-[250] transform transition-transform duration-700 shadow-2xl ${showChat ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-10 border-b border-white/5 flex justify-between items-center bg-black/30"><p className="text-xl font-black text-blue-400 uppercase italic">Messagerie</p><button onClick={() => setShowChat(false)} className="text-3xl text-slate-500">✕</button></div>
        <div className="flex-1 p-8 space-y-6 overflow-y-auto italic text-sm text-slate-400">
           <div className="bg-blue-600/10 p-4 rounded-2xl border border-blue-500/20">
              <span className="text-[10px] font-black text-blue-500 uppercase">Superviseur</span>
              <p>"Wafaa, pense à bien valider les adresses mails."</p>
           </div>
        </div>
        <div className="p-8 bg-black/40 border-t border-white/10 flex gap-4">
          <input type="text" placeholder="Écrire un message..." className="flex-1 bg-white/5 border-2 border-white/5 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-blue-500" />
