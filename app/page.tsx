      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (importedLeads.length > 0) {
            ...
            alert(`${importedLeads.length} leads importés !`);
          }
        },
      });
    }
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="p-3 flex justify-between items-center border-b border-white/5 bg-[#0b0f1a]">
          <nav className="flex gap-1 bg-[#111827] p-1 rounded-lg">
            <button onClick={() => setView("AGENT")} className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase ${view === 'AGENT' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>Agent</button>
