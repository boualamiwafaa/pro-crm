...
import { Phone, Calendar, MessageSquare, Send, X, User, Shield, LogOut, Save } from 'lucide-react';
...
export default function AgentPage() {
  ...
  const [messages, setMessages] = useState([{role: 'Admin', text: 'Bienvenue sur ProCrm !'}]);
    ...
    <div style={{ backgroundColor: '#020617', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ width: '100%', maxWidth: '1100px', backgroundColor: '#1e293b', padding: '15px', borderRadius: '15px', border: '2px solid #3b82f6', display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <Link href="/admin" style={{ backgroundColor: '#7c3aed', padding: '10px 20px', borderRadius: '10px', fontWeight: 'bold', textDecoration: 'none', color: 'white' }}>ADMIN PANEL</Link>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 3fr', gap: '20px', width: '100%', maxWidth: '1100px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          {showKeypad && (
            <div style={{ backgroundColor: '#0f172a', padding: '15px', borderRadius: '15px', border: '2px solid #f59e0b', textAlign: 'center' }}>
              <div style={{ fontSize: '20px', marginBottom: '10px', color: '#f59e0b' }}>06 00 00 00 00</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '5px' }}>
                {[1,2,3,4,5,6,7,8,9,"*",0,"#"].map(n => <button key={n} style={{ padding: '10px', backgroundColor: '#1e293b', border: 'none', color: 'white', borderRadius: '5px' }}>{n}</button>)}
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ backgroundColor: '#0f172a', padding: '30px', borderRadius: '25px', border: '1px solid #334155' }}>
          <h2 style={{ color: '#60a5fa', borderBottom: '1px solid #334155', paddingBottom: '15px' }}>CLIENT : JEAN DURAND</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
            ...
            <textarea placeholder="Commentaires..." style={{ width: '100%', height: '140px', backgroundColor: '#020617', border: '1px solid #334155', padding: '12px', borderRadius: '10px', color: 'white' }}></textarea>
          </div>
          {/* QUALIFICATIONS */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginTop: '20px' }}>
            {["VENTE ✅", "RAPPEL 📞", "REFUS ❌", "NRP"].map(q => (
              <button key={q} style={{ backgroundColor: '#1e293b', padding: '15px', borderRadius: '10px', border: '1px solid #3b82f6', color: 'white', fontWeight: 'bold', cursor: 'pointer' }}>{q}</button>
            ))}
          </div>
        </div>
      </div>

      {/* CHAT AVEC BOUTON ENVOYER ACTIF */}
      {showChat && (
        <div style={{ position: 'fixed', bottom: '20px', right: '20px', width: '320px', backgroundColor: '#0f172a', border: '2px solid #10b981', borderRadius: '15px', zIndex: 100 }}>
          ...
          <div style={{ height: '200px', padding: '15px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {messages.map((m, i) => (
              <div key={i} style={{ alignSelf: m.role === 'Moi' ? 'flex-end' : 'flex-start', backgroundColor: m.role === 'Moi' ? '#065f46' : '#1e293b', padding: '8px', borderRadius: '8px', fontSize: '12px' }}>
                <b>{m.role}:</b> {m.text}
              </div>
            ))}
          </div>
          <div style={{ padding: '12px', borderTop: '1px solid #334155', display: 'flex', gap: '8px' }}>
            ...
            <button onClick={handleSend} style={{ backgroundColor: '#10b981', border: 'none', padding: '10px', borderRadius: '8px', cursor: 'pointer' }}><Send size={18} color="black"/></button>
          </div>
        </div>
      )}
    </div>
