...
import { Phone, Calendar, MessageSquare, Send, X, Save, User, Shield, LogOut } from 'lucide-react';
...
export default function AgentPage() {
  ...
  const addNumber = (n: string | number) => setPhoneNumber(prev => prev + n);
    ...
    <div style={{ backgroundColor: '#020617', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 3fr', gap: '20px', width: '100%', maxWidth: '1100px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          {showKeypad && (
            <div style={{ backgroundColor: '#0f172a', padding: '15px', borderRadius: '15px', border: '2px solid #f59e0b' }}>
              <div style={{ fontSize: '18px', marginBottom: '10px', color: '#f59e0b', fontWeight: 'bold', backgroundColor: '#000', padding: '10px', borderRadius: '8px' }}>{phoneNumber || "Numéro..."}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                {[1,2,3,4,5,6,7,8,9,"*",0,"#"].map(n => (
                  <button key={n} onClick={() => addNumber(n)} style={{ padding: '12px', backgroundColor: '#1e293b', border: 'none', color: 'white', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>{n}</button>
                ))}
              </div>
        ...
        <div style={{ backgroundColor: '#0f172a', padding: '30px', borderRadius: '25px', border: '1px solid #334155' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
               <textarea placeholder="Commentaires..." style={{ width: '100%', height: '100px', backgroundColor: '#020617', border: '1px solid #334155', padding: '12px', borderRadius: '10px', color: 'white', resize: 'none' }}></textarea>
               <button style={{ backgroundColor: '#10b981', color: 'black', padding: '12px', borderRadius: '10px', fontWeight: 'black', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginTop: '20px' }}>
            <button style={{ backgroundColor: '#059669', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>VENTE ✅</button>
            <button onClick={() => setShowCalendar(true)} style={{ backgroundColor: '#2563eb', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>RAPPEL 📞</button>
            <button style={{ backgroundColor: '#991b1b', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>REFUS ❌</button>
            <button style={{ backgroundColor: '#374151', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>NRP</button>
            <button style={{ backgroundColor: '#4b5563', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>BLOCTEL 📵</button>
            <button style={{ backgroundColor: '#4b5563', padding: '12px', borderRadius: '8px', border: 'none', color: 'white', fontWeight: 'bold' }}>HORS CIBLE</button>
          </div>
        </div>
      </div>
      ...
      {showCalendar && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: '#0f172a', padding: '30px', borderRadius: '20px', border: '2px solid #3b82f6', width: '350px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowCalendar(false)} style={{ flex: 1, backgroundColor: '#3b82f6', color: 'white', padding: '12px', borderRadius: '10px', border: 'none', fontWeight: 'bold' }}>VALIDER</button>
              <button onClick={() => setShowCalendar(false)} style={{ flex: 1, backgroundColor: '#ef4444', color: 'white', padding: '12px', borderRadius: '10px', border: 'none', fontWeight: 'bold' }}>ANNULER</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showChat && (
        <div style={{ position: 'fixed', bottom: '20px', right: '20px', width: '320px', backgroundColor: '#0f172a', border: '2px solid #10b981', borderRadius: '15px', zIndex: 100 }}>
          ...
          <div style={{ height: '200px', padding: '15px', overflowY: 'auto' }}>
            <div style={{ backgroundColor: '#1e293b', padding: '8px', borderRadius: '8px', fontSize: '11px' }}><b>Admin:</b> Session ouverte. Bonnes ventes !</div>
          </div>
          <div style={{ padding: '10px', display: 'flex', gap: '5px', borderTop: '1px solid #334155' }}>
            <input value={chatMsg} onChange={(e) => setChatMsg(e.target.value)} type="text" placeholder="Message..." style={{ flex: 1, backgroundColor: '#020617', border: '1px solid #334155', padding: '10px', borderRadius: '8px', color: 'white' }} />
            <button onClick={() => setChatMsg("")} style={{ backgroundColor: '#10b981', padding: '10px', borderRadius: '8px', border: 'none' }}><Send size={18}/></button>
          </div>
        </div>
      )}
    </div>
